...
from .models import Lap, Layout, Sector, Session
...
def match_layout(db, lat: np.ndarray, lon: np.ndarray, speed_mps: np.ndarray, dist_m: np.ndarray):
    ...
    hits.sort(key=lambda h: (-_quality(h[1]), h[0]))
    same_track = [h for h in hits if h[1].track_id == hits[0][1].track_id]
...
def sector_times(df: pd.DataFrame, lap: dict, layout: Layout | None) -> list[dict]:
...
def persist_laps(db, session: Session, df: pd.DataFrame, layout: Layout | None) -> dict | None:
    ...
    for l in laps:
        ...
        db.add(rec)
        db.flush()
        for s in sector_times(df, l, layout):
